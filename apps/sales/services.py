...
class SaleService:
    ...
    @staticmethod
    def listAssignedOrders(data, request):
        data = data or {}
        page = max(int(data.get("page") or 1), 1)
        limit = min(max(int(data.get("limit") or 10), 1), 100)
        offset = (page - 1) * limit
        ...
        queryset = commonQuery.branchScopedQueryset(
            ...
            request,
        ).select_related("customer", "user", "register")
        ...
        items = [
            {
                ...
                "customer__full_name": getattr(order.customer, "full_name", "") if order.customer_id else "",
                "user_id": order.user_id,
                "user__full_name": getattr(order.user, "full_name", "") if order.user_id else "",
                "author_username": getattr(order.user, "username", "") if order.user_id else "",
                "register_id": order.register_id,
                "register__name": getattr(order.register, "name", "") if order.register_id else "",
                "order_type": order.order_type,
                "payment_status": order.payment_status,
                "delivery_status": order.delivery_status,
                "subtotal": order.subtotal,
                "discount_amount": order.discount_amount,
                "total_coupons": order.total_coupons,
